...
class Conv2D(Layer):
    ...
    def __init__(self, filters: int, kernel_size: Union[int, tuple], 
                 ...
                 use_bias: bool = True, key: PRNGKey = PRNGKey(100), 
    ...
    def compute_output_shape(self):
        if self.built:
            return lax.conv_general_shape_tuple(lhs_shape=self.input_shape, rhs_shape=self.kernel_weights.shape, 
            window_strides=self.strides, padding=self.padding, dimension_numbers=self.dimension_numbers)
        else:
    ...
    def build(self, input_shape: Tuple):
        ...
        self.kernel_weights = self.add_weight(key=k1, shape=kernel_shape, initializer=self.kernel_initializer, dtype=self.dtype, name=f'{self.name}_kernel', trainable=self.trainable)
        if self.use_bias:
            bias_shape = self.compute_bias_shape()
            self.bias_weights = self.add_weight(key=k2, shape=bias_shape, initializer=self.bias_initializer, dtype=self.dtype, name=f'{self.name}_bias', trainable=self.trainable)
    ...
    def convolution_op(self, params: Tuple, inputs: DeviceArray):
        output = lax.conv_general_dilated(lhs=inputs, rhs=params[0], window_strides=self.strides, padding=self.padding, dimension_numbers=self.dn)
        output = jnp.add(output, params[1]) if self.use_bias else output
        return lax.cond(self.activation != None, lambda: self.activation(output), lambda: output)
...
class MaxPool2D(Pooling):
    ...
    key: PRNGKey = PRNGKey(1), dtype='float32', name=None, **kwargs):
        super(MaxPool2D, self).__init__(pool_size=pool_size, strides=strides, padding=padding, key=key, dtype=dtype, name=name, expand_dims=True, **kwargs)

    def maxpool_op(self, params: Tuple, inputs: DeviceArray):
        return lax.reduce_window(inputs, -jnp.inf, lax.max, self.pool_size, self.strides, self.padding)
...
class AveragePooling2D(Pooling):
    def __init__(self, pool_size=(2,2), strides=(2,2), padding='valid', key=False, dtype='float32', name=None, **kwargs):
        super(AveragePooling2D, self).__init__(pool_size=pool_size, strides=strides, padding=padding, key=key, dtype=dtype, name=name, expand_dims=False, **kwargs)
    
    
    def rescale(self, outputs, inputs):
        one = jnp.ones((1, inputs.shape[1], inputs.shape[2], 1), dtype=inputs.dtype)
        window_sizes = lax.reduce_window(one, 0., lax.add, self.pool_size, self.strides, self.padding)
        return outputs / window_sizes

    def avgpool_op(self, params: Tuple, inputs: DeviceArray):
        out = lax.reduce_window(inputs, 0., lax.add, self.pool_size, self.strides, self.padding)
