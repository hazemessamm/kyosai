...
class Layer:
    def __init__(
        ...
        **kwargs,
        ...
        self._num_required_inputs = None
        self._num_optional_inputs = None
        self.seed = PRNGKey(layer_utils._check_seed(seed))
        self.trainable = trainable
        self.dtype = dtype or backend.precision()
        self.built = False
    ...
    def parse_inputs(self, *args, **kwargs):
        # Maybe better solution that this.
        if self._required_number_inputs is None or self._num_optional_inputs is None:
            self._num_required_inputs = len(
                list(
                    filter(
                        lambda x: x,
                        [
                            i.default == inspect._empty
                            for i in inspect.signature(self.call).parameters.values()
                        ],
                    )
                )
            )
            self._num_optional_inputs = len(
                list(
                    filter(
                        lambda x: x,
                        [
        ...
        total_num_params = self._num_optional_inputs + self._num_required_inputs

        if total_num_params == 0:
            raise Exception(
                f"Error in layer: {self.name}, `call()` cannot have no parameters."
            )

        requires_unpacking = False
        if total_num_params == len(args):
            if total_num_params == 1:
                if len(kwargs) > 0:
                    raise Exception(f"Unknown arguments: {list(kwargs.keys())}")
                return args[0], kwargs, requires_unpacking
            else:
                if len(kwargs) > 0:
                    raise Exception(f"Unknown arguments: {list(kwargs.keys())}")

                requires_unpacking = True
                return args, kwargs, requires_unpacking
        elif self._num_required_inputs == len(args):
            if self._num_required_inputs == 1:
                return args[0], kwargs, requires_unpacking
            else:
                requires_unpacking = True
                return args, kwargs, requires_unpacking
    ...
    def handle_call(self, inputs, requires_unpacking, **kwargs):
    ...
    def __call__(self, *args, **kwargs):
        inputs, kwargs, require_unpacking = self.parse_inputs(*args, **kwargs)
        if not self.built:
            if isinstance(inputs, Layer):
                self.build(inputs.shape)
                self.connect(inputs)
                ...
                return self.handle_call(
                    inputs, requires_unpacking=require_unpacking, **kwargs
                )
            elif isinstance(inputs, (list, tuple)):
                self.build([_input.shape for _input in inputs])
                if all([isinstance(i, Layer) for i in inputs]):
